    ...
    <div class="py-4 admin-page-info">
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/notifications') ? 'active' : '' }}"
                                        href="{{ route('admin.notifications.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.notifications.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.notifications.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.notifications.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route('admin.notifications.multiAction') }}" method="POST">
                    <div class="pb-3">
                        ...
                        <select id="select-action" class="form-select fmxw-200" name="action"
                            ...
                            <option value="as_read"> Make as read </option>
                        </select>
                    ...
                    <table class="table notification-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom">Notification</th>
                        ...
                        <tbody>
                            @foreach ($notifications as $notification)
                                <tr class="{{$notification->as_read == '0' ? 'not_seen' : '' }} ">
                                    <td>
                                        <div class="form-check dashboard-check">
                                            <input name="id[]" value="{{ $notification->id }}"
                                    ...
                                    <td><span class="fw-normal">{{ $notification->created_at }}</span></td>
                                    <td class="actions">
                                        <a href="{{ $notification->link }}" notification_id="{{ $notification->id }}"
                                            class="text-tertiary notification-link"> <i class="fa-solid fa-eye fa-lg"></i> </a>
                    ...
                    <div
                        ...
                        <div class="d-flex justify-content-center">
                            {{ $notifications->withQueryString()->onEachSide(0)->links() }}
                        </div>
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $notifications->firstItem() }}</b> to <b>{{ $notifications->lastItem() }}</b>
                            of total <b>{{ $notifications->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
