    ...
    <div class="py-4 admin-page-info">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            ...
            @if ( isset($userCount) && $userCount > 0)
                <div class="btn-toolbar mb-2 mb-md-0"><a href="{{ route('admin.prescriptions.create') }}"
                ...
                <div class="col-9 col-lg-8 d-md-flex">
                    <form action="{{ route('admin.prescriptions.search') }}" method="POST" class="input-group me-2 me-lg-3 fmxw-300">
                        ...
                        <input type="email" name="search" class="form-control @error('search') is-invalid @enderror"
                            placeholder="Search prescription by user email"  value='{{ Request::input('search') }}' autocomplete="off" maxlength="55" required/>
                        @error('search')
                        ...
                        <div class="btn-group">
                            <div class="dropdown me-1">
                                ...
                                <div class="dynamic-pagination dropdown-menu dropdown-menu-end pb-0">
                                    ...
                                    <a class="dropdown-item {{ Request::is('*/perPage/10') ? 'active' : '' }} {{ Request::is('admin/prescriptions') ? 'active' : '' }}"
                                        href="{{ route('admin.prescriptions.perPage', 10) }}"> 10 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/30') ? 'active' : '' }}"
                                        href="{{ route('admin.prescriptions.perPage', 30) }}"> 30 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/50') ? 'active' : '' }}"
                                        href="{{ route('admin.prescriptions.perPage', 50) }}"> 50 </a>
                                    <a class="dropdown-item {{ Request::is('*/perPage/100') ? 'active' : '' }}"
                                        href="{{ route('admin.prescriptions.perPage', 100) }}"> 100 </a>
        ...
        @else
            ...
            <div class="card card-body shadow border-0 table-wrapper table-responsive">
                ...
                <!----------- multi Action ------------->
                <form id="multi-action-form" action="{{ route('admin.prescriptions.multiAction') }}" method="POST">
                    <div class="pb-3">
                    ...
                    <table class="table prescription-table table-hover align-items-center index-table">
                        <thead>
                            <tr>
                            ...
                            @foreach ($prescriptions as $prescription)
                                <tr class="{{ $prescription->validation == '1' ? 'pending' : '' }}">
                                    ...
                                    <td class="actions">
                                        ...
                                        <a href="{{ route('admin.prescriptions.destroy', $prescription->id) }}"
                    ...
                    <div
                        ...
                        <div class="d-flex justify-content-center">
                            {{ $prescriptions->withQueryString()->onEachSide(0)->links() }}
                        </div>
                        <div class="fw-normal small mt-4 mt-lg-0">
                            Showing <b>{{ $prescriptions->firstItem() }}</b> to <b>{{ $prescriptions->lastItem() }}</b>
                            of total <b>{{ $prescriptions->total() }}</b> entries
                        </div>
                    </div>
                </form>
            </div>
